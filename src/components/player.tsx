...
function Player({ video, isMiniPlayer = false, setTranscript, progress, route, db }: PlayerProps) {
  ...
  const [tracks, setTracks] = useState<{id: string, key: string, kind: string, src: string, srcLang: string, label: string, dataType: string}[]>([]);

    const fetchSubtitles = async (subtitles:Subtitle[]) => {
      console.time("fetching subtitles")
      const newTracks = await Promise.all(subtitles.map(async (subtitle) => {
        if (!subtitle.url) return null;
        if (subtitle.mimeType !== "application/ttml+xml") return {
